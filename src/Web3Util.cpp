...
uint32_t Web3Util::RlpEncodeWholeHeader(uint8_t *header_output, uint32_t total_len)
{
    if (total_len < 55)
    {
...
vector<uint8_t> Web3Util::RlpEncodeWholeHeaderWithVector(uint32_t total_len)
{
...
uint32_t Web3Util::RlpEncodeItem(uint8_t *output, const uint8_t *input, uint32_t input_len)
{
...
vector<uint8_t> Web3Util::RlpEncodeItemWithVector(const vector<uint8_t> input)
{
...
vector<uint8_t> Web3Util::ConvertNumberToVector(unsigned long long val)
{
    vector<uint8_t> tmp;
    vector<uint8_t> ret;
...
vector<uint8_t> Web3Util::ConvertNumberToVector(uint32_t val)
{
    return ConvertNumberToVector((unsigned long long)val);
}

uint32_t Web3Util::ConvertNumberToUintArray(uint8_t *str, uint32_t val)
{
    uint32_t ret = 0;
    uint8_t tmp[8];
    memset(tmp, 0, 8);
...
uint8_t Web3Util::ConvertCharToByte(const uint8_t *ptr)
{
    char c[3];
    c[0] = *(ptr);
    c[1] = *(ptr + 1);
...
vector<uint8_t> Web3Util::ConvertHexToVector(const uint8_t *in)
{
...
vector<uint8_t> Web3Util::ConvertHexToVector(const string *str)
{
    return ConvertHexToVector((uint8_t *)(str->c_str()));
}

uint32_t Web3Util::ConvertCharStrToUintArray(uint8_t *out, const uint8_t *in)
{
    uint32_t ret = 0;
    const uint8_t *ptr = in;
    // remove "0x"
...
uint8_t Web3Util::HexToInt(uint8_t s)
{
    uint8_t ret = 0;
    if (s >= '0' && s <= '9')
    {
...
string Web3Util::VectorToString(const vector<uint8_t> *buf)
{
    return ConvertBytesToHex((const uint8_t *)buf->data(), buf->size());
}

string Web3Util::ConvertIntegerToBytes(const int32_t value)
{
    size_t hex_len = 4 << 1;
...
string Web3Util::PlainVectorToString(const vector<uint8_t> *buf)
{
...
string Web3Util::ConvertBytesToHex(const uint8_t *bytes, int length)
{
...
void Web3Util::ConvertHexToBytes(uint8_t *_dst, const char *_src, int length)
{
...
string Web3Util::ConvertBase(int from, int to, const char *s)
{
    if (s == NULL)
        return NULL;
...
string Web3Util::ConvertDecimal(int decimals, string *result)
{
...
string Web3Util::ConvertHexToASCII(const char *result, size_t length)
{
    ...
    while (index < length && endOfString == false)
    {
        ...
        {
        case 0:
            reader = (char)(Web3Util::HexToInt(c) * 16);
            state = 1;
            break;
        case 1:
            reader += (char)Web3Util::HexToInt(c);
            if (reader == 0)
            {
                endOfString = true;
            }
 ...
 **/
vector<string> *Web3Util::ConvertCharStrToVector32(const char *resultPtr, size_t resultSize, vector<string> *result)
{
    if (resultSize < 64)
        return result;
 ...
 */
vector<string> *Web3Util::ConvertResultToArray(string *value)
{
    TagReader reader;
...
vector<string> *Web3Util::ConvertStringHexToABIArray(string *result)
{
    int startIndex = 0;
    vector<string> *retVal = new vector<string>();
    if (result->at(0) == 'x')
        startIndex = 1;
    else if (result->at(1) == 'x')
        startIndex = 2;
...
string Web3Util::InterpretStringResult(const char *result)
{
    ...
    if (result != NULL && strlen(result) > 0)
    {
        ...
        Web3Util::ConvertCharStrToVector32(result, strlen(result), &breakDown);
...
vector<string> *Web3Util::InterpretVectorResult(string *result)
{
    vector<string> *retVal = new vector<string>();
    TagReader reader;
    ...
    {
        vector<string> breakDown;
        Web3Util::ConvertCharStrToVector32(value, reader.length(), &breakDown);
...
void Web3Util::PadForward(string *target, int targetSize)
{
...
uint256_t Web3Util::ConvertToWei(double val, int decimals)
{
    ...
    if (index != std::string::npos)
        weiStr = weiStr.substr(0, index);
    weiStr = ConvertBase(10, 16, weiStr.c_str());
    return uint256_t(weiStr.c_str());
}

string Web3Util::ConvertWeiToEthString(uint256_t *weiVal, int decimals)
{
    char buffer[65];
...
string Web3Util::ConvertEthToWei(double eth)
{
    char buffer[36]; // allow extra 4 chars for gcvt
    if (eth < 0)
        eth = 0;
...
string Web3Util::toString(int value)
{
...
string Web3Util::intToHex(int value)
{
