...
Crypto::Crypto(Web3JBC *_web3)
{
    web3 = _web3;
    memset(privateKey, 0, ETHERS_PRIVATEKEY_LENGTH);
}

bool Crypto::Sign(uint8_t *digest, uint8_t *result)
{
...
{
    Web3JBCUtil::ConvertHexToBytes(privateKey, key, ETHERS_PRIVATEKEY_LENGTH);
}

void Crypto::ECRecover(uint8_t *signature, uint8_t *public_key, uint8_t *message_hash)
{
    uint8_t v = signature[64]; // recover 'v' from end of signature
    uint8_t pubkey[65];
    uint8_t signature64[64];
    if (v > 26)
        v -= 27; // correct value of 'v' if required
...
String Crypto::Keccak256(vector<uint8_t> *bytes)
{
    ...
    return Web3JBCUtil::VectorToString(&resultVector);
}

String Crypto::ECRecoverFromHexMessage(const String &signature, const String &message)
{
    uint8_t challengeHash[ETHERS_KECCAK256_LENGTH];
    // convert hex to bytes
    vector<uint8_t> messageBytes = Web3JBCUtil::ConvertHexToVector((uint8_t *)(message.c_str()));
...
String Crypto::ECRecoverFromHash(const String &signature, uint8_t *digest)
{
    // convert address to uint8_t
    uint8_t signatureBytes[ETHERS_SIGNATURE_LENGTH];
    uint8_t publicKeyBytes[ETHERS_PUBLICKEY_LENGTH];

    Web3JBCUtil::ConvertHexToBytes(signatureBytes, signature.c_str(), ETHERS_SIGNATURE_LENGTH);
    ...
    uint8_t recoverAddr[ETHERS_ADDRESS_LENGTH];
    // Now reduce the recovered public key to Ethereum address
    Crypto::PublicKeyToAddress(publicKeyBytes, recoverAddr);
    return Web3JBCUtil::ConvertBytesToHex(recoverAddr, ETHERS_ADDRESS_LENGTH);
}

String Crypto::ECRecoverFromPersonalMessage(const String &signature, const String &message)
{
    uint8_t challengeHash[ETHERS_KECCAK256_LENGTH];
    String challengeStr = PERSONAL_MESSAGE_PREFIX;
    challengeStr += String((int)message.length()).c_str();
